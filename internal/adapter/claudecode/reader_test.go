...
// An oversized line does not gate this rule: maxLineBytes is a fixed internal
// constant, so the line is oversized on every future scan and deferring never
// resolves it (see the comment above result.Malformed += skipped in Read).
func TestReadResolvesASkillCandidateDespiteAnOversizedLine(t *testing.T) {
	...
	if len(result.Records) != 1 || result.AmbiguousSkillRuns != 0 {
		t.Fatalf("Read() = %+v, want the fallback resolved", result)
	}
	if result.Malformed != 1 {
		t.Errorf("Malformed = %d, want 1 — the blindness doctor reports", result.Malformed)
	}
	if result.OpenSessions != 0 {
		t.Errorf("OpenSessions = %d, want 0 — an oversized line no longer blocks the staleness judgement", result.OpenSessions)
	}
}

// A read that could not rule out a genuinely unreadable line may not conclude a
// session went silent, so it may not emit a Shape-A fallback either. That record
// is permanent — ADR-0015 rejects upsert and ADR-0004 deduplicates the correction
// away — and deriving it from a line the reader is blind to is what plan §3.3
// forbids. The candidate stays buffered and the session stays open, which costs a
// later scan and never a wrong record.
func TestReadDoesNotResolveASkillCandidateWhenALineWasUnreadable(t *testing.T) {
	candidate := `{"uuid":"entry-1","sessionId":"session-1","cwd":"/repo","timestamp":"2026-08-13T12:00:00Z","attributionSkill":"pr-review","message":{"model":"sonnet","stop_reason":"end_turn"}}`
...
// An oversized line does not gate the staleness rule: maxLineBytes is a fixed
// internal constant, so the line is oversized on every future scan and deferring
// never resolves it (see the comment above result.Malformed += skipped in Read).
...
func TestReadInterruptsAStaleCallDespiteAnOversizedLine(t *testing.T) {
	...
	if len(result.Records) != 1 || result.Interrupted != 1 || result.Pending != 0 {
		t.Fatalf("Read() = %+v, want the stale call resolved to interrupted", result)
	}
}
...
// An oversized line does not gate the cursor floor either: maxLineBytes is a
// fixed internal constant, so deferring the judgement never resolves it (see the
// comment above result.Malformed += skipped in Read). The session is judged on
...
func TestReadResolvesSessionsDespiteAnOversizedLine(t *testing.T) {
	...
	if result.OpenSessions != 0 {
		t.Errorf("OpenSessions = %d, want 0", result.OpenSessions)
	}
...
func TestReadKeepsEverySessionOpenWhenALineWasUnreadable(t *testing.T) {
	// An unreadable line is never observed as activity, so every session's last
	// activity is known-understated once the reader could not attribute one —
	// including a session whose only recent line is the one it could not read.
	// Reporting such a session closed would let a future cursor (ADR-0023 §5)
	// advance past a session that is still alive, so the whole read declines to
