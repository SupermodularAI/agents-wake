...
// Exactly one of each, not "at least one". A second registration is the failure
// this test exists for: it would print doctor's remote section twice and spawn
// two detached flushes per scan, and neither is loud enough to notice without an
// assertion.
func TestEachSeamIsRegisteredExactlyOnce(t *testing.T) {
	if len(diagnosisSections) != 1 {
		t.Errorf("registered %d diagnosis sections, want exactly 1", len(diagnosisSections))
	}
	if len(afterScan) != 1 {
